...
local config = require "nvim.bfs.config"
local keymaps = require("nvim.bfs.keymaps")
...
function M.selBufNum(win, opt, count)
    ...
    if count ~= 0 then
        ...
        if filename:split("_", true)[1] == "Terminal" then filename = M.terminal_map[filename] end
    end
...
M.set_buffers = function(buf)
    for i, b in ipairs(get_bufs()) do
        ...
        if filename:split(" ", true)[1] == "Terminal:" then
            hl_group = hl_group .. "term"
            file_icon, file_icon_color = require("nvim-web-devicons").get_icon_color("terminal", extension,
                                                                                     {default = true})
        end
